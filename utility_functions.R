...
fit_one_gaussian_and_return_parameters <- function(df, feature) {
  gaussian_parameters <- data.frame()
  ...
  for(sample_id in unique(df$sample_id)) {
    ...
    m <- nlsLM(DensityFaithfulY ~ gauss(DensityFaithfulX, mu, sigma, A), 
               start = list(mu = wait.summary.df[["mu"]][1], sigma  = wait.summary.df[["std"]][1], 
                            A = wait.summary.df[["alpha"]][1]),
               weights = 1 / (DensityFaithfulX - 2 * DensityFaithfulX[1] + DensityFaithfulX[2]))
    
    m_res <- coef(m)
    
    gaussian_parameters <- rbind(gaussian_parameters, data.frame(sample_id = sample_id, mu =  m_res[1], sigma = m_res[2]))
  }
  
  rownames(gaussian_parameters) <- gaussian_parameters$sample_id
  gaussian_parameters <- gaussian_parameters[-1]
